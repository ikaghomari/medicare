...
<?php 
  ...
  $rapport=$_POST['rapport'];
    ...
    if( !empty($Num)){
        ...
        if (mysqli_connect_error()) {
          ...
          $qry = "UPDATE patient SET  rapport='$rapport' , dateC='$dateC' , consult='$consult'  WHERE Num=$Num ";
          $result = mysqli_query($conn,$qry); //query executes

           $qry = "UPDATE patient SET medecinS = medecinS FROM patient INNER JOIN service ON service=service WHERE Num=$Num ";
       
          $result = mysqli_query($conn,$qry); //query executes


            echo '<h2>Ces informations ont été bien ajoutées dans le dossier</h2> ';
          }
 ...
 ?>
  ...
  <form method="POST" action="affdossier.php">
       <br>
      <input type="hidden" name="Num" class="form-field animation a3" value="<?php echo "$Num"?>"> <br>
      <button type="submit" class="form-field animation a4">afficher le dossier</button>
